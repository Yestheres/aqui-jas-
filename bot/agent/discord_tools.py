...
def _permission_for(action: str) -> str:
    return {
        "create_channel": "manage_channels", "update_channel": "manage_channels",
        "delete_channel": "manage_channels", "move_channel": "manage_channels",
        "set_channel_permissions": "manage_channels", "sync_channel_permissions": "manage_channels",
        "create_role": "manage_roles", "update_role": "manage_roles", "delete_role": "manage_roles",
        "assign_role": "manage_roles", "remove_role": "manage_roles",
        "update_nickname": "manage_nicknames", "timeout_member": "moderate_members",
        "kick_member": "kick_members", "ban_member": "ban_members", "unban_member": "ban_members",
...
def _overwrite_input(data: dict[str, Any]) -> discord.PermissionOverwrite:
    allowed = {str(name): True for name in data.get("allow", [])}
    denied = {str(name): False for name in data.get("deny", [])}
    allowed.update(denied)
    return discord.PermissionOverwrite(**allowed)


async def execute_action(guild: discord.Guild, action_type: str, target: dict[str, Any], input_data: dict[str, Any]) -> dict[str, Any]:
    ...
    if action_type == "create_channel":
        ...
        if kind == "voice" and isinstance(existing, discord.VoiceChannel):
            return {"channel_id": existing.id, "reused": True}
        if kind == "category": channel = await guild.create_category(name, reason=reason)
        elif kind == "voice": channel = await guild.create_voice_channel(name, reason=reason)
        else: channel = await guild.create_text_channel(name, reason=reason)
    ...
    if action_type == "update_channel":
        ...
        if channel is None: raise ValueError("Canal não encontrado.")
        kwargs: dict[str, Any] = {}
        if "name" in input_data: kwargs["name"] = str(input_data["name"])
        if "topic" in input_data and isinstance(channel, discord.TextChannel): kwargs["topic"] = str(input_data["topic"])
        if "nsfw" in input_data and isinstance(channel, discord.TextChannel): kwargs["nsfw"] = bool(input_data["nsfw"])
        if "rate_limit_per_user" in input_data and isinstance(channel, discord.TextChannel): kwargs["slowmode_delay"] = max(0, min(21600, int(input_data["rate_limit_per_user"])))
        if kwargs: await channel.edit(**kwargs, reason=reason)
    ...
    if action_type == "move_channel":
        ...
        if channel is None or not isinstance(category, discord.CategoryChannel): raise ValueError("Canal ou categoria destino não encontrada.")
        if channel.category_id == category.id: return {"channel_id": channel.id, "parent_category_id": category.id, "changed": False}
    ...
    if action_type == "set_channel_permissions":
        ...
        if subject is None: raise ValueError("Alvo de permissão não encontrado.")
    ...
    if action_type == "delete_channel":
        channel = guild.get_channel(_target_int(target, "channel_id"))
        if channel is None: return {"deleted": False, "already_missing": True}
        await channel.delete(reason=reason)
        return {"deleted": True, "channel_id": channel.id}

    if action_type == "create_role":
        name = str(input_data.get("name", "")).strip()
        if not name: raise ValueError("input.name é obrigatório.")
        existing = discord.utils.find(lambda r: r.name == name, guild.roles)
        if existing and existing != guild.default_role: return {"role_id": existing.id, "reused": True}
        raw_color = str(input_data.get("color", "0")).lstrip("#")
        try: color = discord.Colour(int(raw_color, 16)) if raw_color else discord.Colour.default()
        except ValueError as exc: raise ValueError("input.color inválida.") from exc
    ...
    if action_type == "update_role":
        ...
        if role is None: raise ValueError("Cargo não encontrado.")
        if role >= guild.me.top_role: raise RuntimeError("O cargo alvo está acima ou no mesmo nível do cargo do bot.")
        kwargs: dict[str, Any] = {}
        if "name" in input_data: kwargs["name"] = str(input_data["name"])
        if "mentionable" in input_data: kwargs["mentionable"] = bool(input_data["mentionable"])
        if "color" in input_data:
            kwargs["colour"] = discord.Colour(int(str(input_data["color"]).lstrip("#"), 16))
        if kwargs: await role.edit(**kwargs, reason=reason)
        return {"role_id": role.id, "updated": bool(kwargs)}

    if action_type == "delete_role":
        role = guild.get_role(_target_int(target, "role_id"))
        if role is None: return {"deleted": False, "already_missing": True}
        if role >= guild.me.top_role: raise RuntimeError("O cargo alvo está acima ou no mesmo nível do cargo do bot.")
    ...
    if action_type in {"assign_role", "remove_role"}:
        ...
        if role is None: raise ValueError("Cargo não encontrado.")
        if role >= guild.me.top_role: raise RuntimeError("O cargo alvo está acima ou no mesmo nível do cargo do bot.")
        if action_type == "assign_role": await member.add_roles(role, reason=reason)
    ...
    if action_type == "update_nickname":
        member = guild.get_member(_target_int(target, "member_id")) or await guild.fetch_member(_target_int(target, "member_id"))
        if member != guild.me and member.top_role >= guild.me.top_role: raise RuntimeError("Hierarquia impede alterar este membro.")
    ...
    if action_type == "timeout_member":
        member = guild.get_member(_target_int(target, "member_id")) or await guild.fetch_member(_target_int(target, "member_id"))
        if member != guild.me and member.top_role >= guild.me.top_role: raise RuntimeError("Hierarquia impede timeout neste membro.")
    ...
    if action_type == "kick_member":
        member = guild.get_member(_target_int(target, "member_id")) or await guild.fetch_member(_target_int(target, "member_id"))
        if member.top_role >= guild.me.top_role: raise RuntimeError("Hierarquia impede expulsar este membro.")
        await member.kick(reason=reason)
        return {"member_id": member.id, "kicked": True}

    if action_type == "ban_member":
        member = guild.get_member(_target_int(target, "member_id")) or await guild.fetch_member(_target_int(target, "member_id"))
        if member.top_role >= guild.me.top_role: raise RuntimeError("Hierarquia impede banir este membro.")
    ...
    if action_type == "send_message":
        channel = guild.get_channel(_target_int(target, "channel_id"))
        if channel is None or not isinstance(channel, discord.abc.Messageable): raise ValueError("Canal não encontrado ou não aceita mensagens.")
        content = str(input_data.get("content", "")).strip()
        if not content: raise ValueError("input.content é obrigatório.")
