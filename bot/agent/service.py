...
from .plan import AgentPlan, ActionSpec, parse_plan
from .prompt import SYSTEM_PROMPT, build_prompt
from .validator import resolve_refs, validate_plan
...
class AgentService:
    ...
    def guild_context(guild: discord.Guild) -> str:
        ...
        return f"name={guild.name!r}\nmembers={guild.member_count or 0}\nchannels:\n" + "\n".join(channels) + "\nroles:\n" + "\n".join(roles)

    async def generate_plan(self, guild: discord.Guild, request: str) -> AgentPlan:
        config = await self.bot.db.get_ai_config(guild.id)
        if config is None:
            raise RuntimeError("Este servidor ainda não configurou uma IA. Use `/configia`.")
        api_key, base_url, model = config
        client = AIClient(AIConfig(api_key=api_key, base_url=base_url, model=model))
        raw = await client.chat(SYSTEM_PROMPT, build_prompt(self.guild_context(guild), request, guild.id))
        plan = parse_plan(raw, guild.id)
        validate_plan(plan)
        return plan

    async def execute(self, guild: discord.Guild, user_id: int, plan: AgentPlan) -> list[dict[str, Any]]:
        ...
        for action in plan.actions:
            ...
            try:
                ...
                report.append({"id": action.id, "type": action.type, "status": "executed", "result": value})
                await self.bot.db.log_action(guild.id, user_id, action.type, action.risk, "executed", json.dumps(value, ensure_ascii=False, default=str))
            except Exception as exc:
                await self.bot.db.log_action(guild.id, user_id, action.type, action.risk, "failed", str(exc))
                report.append({"id": action.id, "type": action.type, "status": "failed", "error": str(exc)})
                if plan.on_error == "stop_and_report":
                    break
        return report


def action_line(action: ActionSpec) -> str:
